...
async function main() {
    ...
    // Mint new NFTs from the collection using custom function safeMint()
    console.log('Minting a new NFT from the collection to the contractOwner...');
    const contractOwner = signers[0].address;
    const initialMintCount = 5; // Number of NFTs to mint
    let initialMint = [];
    for (let i = 1; i <= initialMintCount; i++) {
        let tx = await nonFunToken.safeMint(signers[0].address, i.toString());
    ...
    for (let i = 0; i < initialMint.length - 1; i++) {
